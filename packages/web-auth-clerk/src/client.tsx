...
export const CLERK_APPEARANCE = Object.freeze({
  ...
  elements: {
    cardBox: "shadow-none",
    card: "shadow-none border border-solid",
  },
});
...
/**
 * Mounts Clerk for normal operation. A keyless development bypass renders its
 * children without a provider so test and static-render lanes never invent a
...
export function ClerkSignInBlock({
  appearance,
  redirect_url,
  eyebrow,
  heading,
  subtitle,
  signup_href,
  signup_label = "Create an account",
}: ClerkSignInProps) {
  return (
    <section aria-labelledby="clerk-sign-in-heading">
      ...
      <SignIn
        appearance={appearance ?? CLERK_APPEARANCE}
        fallbackRedirectUrl={redirect_url}
        signUpUrl={signup_href}
      />
...
export function humaniseClerkError(error: unknown): string {
  ...
  const direct = nonEmptyString(first?.longMessage)
    ?? nonEmptyString(first?.message)
    ?? nonEmptyString(shaped.message);
