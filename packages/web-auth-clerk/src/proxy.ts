...
async function finishResponse(
  request: NextRequest,
  config: Pick<SiteProxyConfig, "afterAuth" | "decorateResponse">,
): Promise<Response> {
...
export function createSiteProxy(config: SiteProxyConfig = {}): NextMiddleware {
  ...
  const isProtected = config.protectedRoutes
    ...
    : (() => {
        const isPublic = createRouteMatcher(config.publicRoutes ?? DEFAULT_PUBLIC_ROUTES);
        return (request: NextRequest) => !isPublic(request);
      })();

  const handler = async (auth: ClerkMiddlewareAuth, request: NextRequest) => {
    if (!devAuthBypassEnabled() && isProtected(request)) {
      await auth.protect();
    }
