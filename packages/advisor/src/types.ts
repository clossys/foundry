...
export type AdvisorSession = | { id: string; state: Exclude<AdvisorSessionState, "closed">; nextAction: EngagementNextAction; lastAssessment: AdvisorAssessment | null; closure?: never } | { id: string; state: "closed"; lastAssessment: AdvisorAssessment | null; closure: SessionClosure; nextAction?: never };
...
export type AdvisorSessionEvent = | { type: "assessment-recorded"; assessment: AdvisorAssessment; nextAction: EngagementNextAction } | { type: "sponsor-approved"; authorization: ExecutionAuthorization; asOf: string; nextAction: EngagementNextAction } | { type: "reassess"; nextAction: EngagementNextAction } | { type: "close"; reason: string; evidence: readonly EvidenceReference[] };

export interface EngagementDecisionCurrencyInput { asOf: string; engagements: readonly EngagementRecord[]; assessments: readonly AdvisorAssessment[]; elapsedDaysByEngagement: Readonly<Record<string, number>>; }
