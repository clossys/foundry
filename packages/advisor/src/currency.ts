...
export function assessEngagementDecisionCurrency(input: EngagementDecisionCurrencyInput): EngagementDecisionCurrencyAssessment {
  ...
  const assessments = input.assessmentInputs.map((assessmentInput) => ({ engagementId: engagementIdOf(assessmentInput), assessment: assessAdvisorEngagement(assessmentInput) }));
  ...
  for (const engagement of active) {
    ...
    const assessedAt = Date.parse(engagement.assessmentBasis.assessedAt); const freshUntil = Date.parse(engagement.assessmentBasis.freshUntil);
    if (Number.isNaN(assessedAt) || Number.isNaN(freshUntil) || freshUntil <= assessedAt || (!Number.isNaN(asOf) && (asOf < assessedAt || asOf >= freshUntil))) { findings.push(finding("assessment-basis-stale", "The assessment basis is invalid, inverted, or outside its explicit freshness window.", engagement.id)); valid = false; }
