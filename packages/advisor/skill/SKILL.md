...
8. Record the choice by proposing one step for their approval: Launcher writes the inventory, `npx @clossys/launcher --repositories <id>,<id>` with the chosen ids in the order the check returned them, run from the hub, or from the repository being appointed as the hub. The inventory Launcher writes is the record: never write `clossys/.state/inventory.json` yourself and keep no second list. If Launcher refuses because the hub's inventory already lists different repositories, tell them in plain words which would be added and which removed, and run again with `--replace-inventory` only after they say yes to that.
