...
1. Check the GitHub command-line tool is signed in: `gh auth status`. If `gh` is missing or not signed in, say so plainly -- "I can't see your GitHub repositories yet, because the GitHub command-line tool isn't signed in on this computer" -- tell them `gh auth login` is the one step they run themselves, and stop. Never ask for a token, a password, or a pasted key, and never offer to type repository names instead.
2. List what their account can see, with only these two fields: `gh repo list --no-archived --limit 200 --json nameWithOwner,description`, and the same for each organization `gh org list` names (`gh repo list <organization> ...`). Combine the results into one JSON array.
3. Build the card from that list with `repositoryChoiceCard(listing, { current })`, where `current` is the repository they are in (`gh repo view --json nameWithOwner --jq .nameWithOwner`). Before a hub exists, the package is not installed here: run `npx -p @clossys/advisor advisor-repository-card repositories.json --current <owner/name>` instead. If the list is refused, say you could not read their repository list and try once more; never repair it by hand, and never repeat a name from a finding.
4. Ask the card like every other card: one question, the recommended repository first, and they may choose several. Show each choice's label and, when there is one, its description.
5. Check their answer with `applyRepositoryChoice(card, chosen)` (or `advisor-repository-card repositories.json --choose <id>,<id>`). A refused choice means asking again. `something-else` means a repository they expected is missing: ask the card's follow-up, and list again once they have access.
6. Record the choice by proposing one step for their approval: Launcher writes the inventory, `npx @clossys/launcher --repositories <id>,<id>` with the chosen ids in the order the check returned them, run from the hub, or from the repository being appointed as the hub. The inventory Launcher writes is the record: never write `clossys/.state/inventory.json` yourself and keep no second list. If Launcher refuses because the hub's inventory already lists different repositories, tell them in plain words which would be added and which removed, and run again with `--replace-inventory` only after they say yes to that.
