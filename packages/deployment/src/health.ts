...
export function evaluateDeploymentHealth(observations: readonly DeploymentObservation[]): DeploymentHealthSummary {
  ...
  for (const observation of observations) {
    if (statuses.includes(observation.status)) counts[observation.status] += 1;
  }
  const status: DeploymentHealthStatus = counts.unhealthy > 0 ? "unhealthy" : counts.degraded > 0 ? "degraded" : counts.unknown > 0 || observations.length === 0 ? "unknown" : "healthy";
