/**
 * @vespeneventures/policy — a zero-dependency package in this small foundation.
 *
 ...
 * this package provides — compute a digest, validate a binding's shape,
 * verify a binding against materialized content.
 *
 * `Finding` is this package's own type, defined in `./types.js` alongside
 * `PolicyBinding` — not borrowed from, or shared with, any other package.
 *
 * NON-GOAL, deliberately: this package does not know what a "denylist" is,
 ...
 * whatever calls this, most likely a `gates`-layer package higher up this
 * same foundation. This is a general-purpose content-addressed binding
 * primitive; a denylist is its motivating use case, not something it has
