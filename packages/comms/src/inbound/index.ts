/**
 * Inbound webhook admission doctrine — deliberately NOT an HTTP handler.
 *
 * This mirrors the ownership split `comms` already uses for
 * `DeliveryEventLedger`: the host implements a ledger interface and owns
 * the transport; foundry owns the decision logic on top of it.
 *
 * - The consumer owns the HTTP route, raw-body access, and SIGNATURE
 *   VERIFICATION. Signature schemes are provider-specific — foundry cannot
 *   test a provider's signing algorithm against a real secret and must not
 *   pretend to verify what it cannot exercise.
 * - foundry owns the ADMISSION DECISION: dedupe, ack/reject doctrine,
 ...
 * Mirrors `DeliveryEventLedger.apply()`: provider webhooks are
 * at-least-once and may be delivered more than once and out of order, so
