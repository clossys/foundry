 ...
 * and never thrown as though it were a real, actionable violation.
 ...
 * this package's own `package.json`, which declares no dependency on it),
 * so importing controller's type from here would invert that graph — an
 * architectural decision for an owner, not a bug-fix detail.
...
type RangeSatisfaction =
   ...
   * any value carrying a prerelease identifier (`6.19.0-rc.1`), build
 ...
 * runs at MODULE LOAD, so a consumer that imports this package's `resend`
 * subpath more than once would otherwise see the identical warning
...
export function assertPeerVersion(input: AssertPeerVersionInput): void {
  ...
  if (!outcome.evaluated && outcome.kind === "unparseable-version") {
    ...
    if (!warnedUnparseableVersions.has(warnKey)) {
      ...
      console.warn(
        `[@vespeneventures/comms] Could not verify ${peer}@${foundVersion} against this package's declared range ` +
