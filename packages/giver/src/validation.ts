 ...
 * human-readable location like `"handoffs[2].sla.minutes"`), and `issues`
 ...
 * recorded into `issues` at the moment it is discovered, so the standard
 ...
 * ---------------------------------------
 * No helper in this file substitutes a value for one it could not read.
 * That is the rule the whole package exists to hold: an absent or
 * unparseable input becomes a recorded failure, never a default that reads
 * as a positive answer downstream. A timestamp that cannot be parsed is a
 * validation failure, not a silent `Date.now()`; an absent boolean is a
 * failure, not `false`.
 */

export interface ValidationIssue {
  /** Absolute, dot/bracket-joined location, e.g. `"proofs[0].state"`, or `"(root)"` for a whole-value shape problem. */
  path: string;
  message: string;
}
 ...
 * — never the value's full (potentially huge, potentially
 ...
 * treated as "now" — both the placement gate and the discharge gate are
 * arithmetic on these values, and a fabricated one would turn a hand-off
 * nobody picked up into one placed on time.
 */
 ...
 * file follows, so an entity's own per-object reader (e.g.
 * `readHandoffRecord` in `schema.ts`) plugs directly into this without an
 * adapter.
 */
