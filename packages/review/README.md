...
const evidence: ReviewEvidenceBundle = {
  ...
  reviews: [{ id: "review-1", state: "approved", headSha: "0123456789abcdef0123456789abcdef01234567" }],
...
current thread remains unresolved. Dismissed reviews are retained as evidence
but do not request changes or count as approvals.
...
| `ReviewDecision` / `ReviewRecord` | types | A review state and current-head decision record. |
