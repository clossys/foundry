...
const REVIEW_KEYS = new Set(["id", "state", "headSha"]);
...
function validateReviews(entries: unknown[], headSha: string, findings: ReviewFinding[]): { hasApproval: boolean; hasChangesRequested: boolean } {
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    const { id, state, headSha: itemHeadSha } = entry;
    ...
    if (state === "changes-requested") hasChangesRequested = true;
  }
...
export function isReviewEvidenceBundle(value: unknown): value is ReviewEvidenceBundle {
  return validateReviewEvidence(value, { requiredChecks: [], requireApproval: false }).length === 0;
}
