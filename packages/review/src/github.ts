...
export interface GitHubReviewNode { readonly id: string; readonly state: string | null; readonly commit?: { readonly oid?: string | null } | null; readonly commit_id?: string | null; }
...
export function normalizeGitHubReviewEvidence(payload: GitHubReviewEvidencePayload): ReviewEvidenceBundle {
  ...
  return {
    ...
    reviews: nodes(reviewsConnection).map((node) => { const review = record(node); const commit = record(review.commit); return { id: stringValue(review.id), state: normalizeReviewDecision(review.state), headSha: stringValue(commit.oid) || stringValue(review.commit_id) || headSha }; }),
