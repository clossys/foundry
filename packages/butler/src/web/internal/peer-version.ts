 ...
 * reported once via `console.warn` and never thrown as though it were a
 * real, actionable violation. An unparseable DECLARED RANGE is unchanged —
 * that range is this package's own source, not external input, so failing
 * to parse it is this package's own bug and still throws.
 */
...
export function assertPeerVersion(input: AssertPeerVersionInput): void {
  ...
  if (!outcome.evaluated && outcome.kind === "unparseable-version") {
    ...
    if (!warnedUnparseableVersions.has(warnKey)) {
      ...
      console.warn(
        `[@clossys/butler] Could not verify ${peer}@${foundVersion} against this package's declared range ` +
