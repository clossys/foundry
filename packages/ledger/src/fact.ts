 ...
 * `@vespeneventures/policy`'s own `computeDigest` to build a `FactCitation`
 ...
 * same producer-side-error precedent `@vespeneventures/policy`'s own
 ...
 * `@vespeneventures/policy`'s own default) via `computeDigest`, and returns
 ...
 * `@vespeneventures/policy`'s `verifyBinding` for that half so the compare
