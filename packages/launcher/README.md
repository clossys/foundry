...
- A repository whose default branch declares a Controller repository
  ...
  a root name the set introduces (`unowned-existing`), gets the item
  refused instead; a profile that already declares every name, or checks no
  root vocabulary, gets no item.
