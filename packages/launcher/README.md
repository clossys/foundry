...
```bash
npm install --save-dev --save-exact @clossys/launcher@0.1.0
```
...
| Any other GitHub repository you control | Appoints it as the account hub. Keeps the existing name and files. Writes the hub marker, pins `@clossys/advisor` as a devDependency, and leaves the rest of the tree alone. |
...
the product. Sister repositories still pin their own roles.
...
| Export | Description |
| --- | --- |
| `planWorkspace()` | Decides create, resume, or adopt from a cwd observation. |
| `applyWorkspacePlan()` | Copies the in-package skeleton or hub marker through a host port. |
| `observeWorkspace()` | Reads `gh`, git remotes, cwd, and the public Advisor version. |
...
| `CommandResult` / `CwdObservation` / `HubDocument` / `WorkspaceDecision` / `WorkspaceHost` / `WorkspaceObservation` / `WorkspacePlan` / `WorkspaceRefusal` / `WorkspaceState` | Typed host, observation, plan, and outcome contracts. |
