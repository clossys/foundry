...
- Appoint merges `--inventory` into a populated on-disk hub inventory instead of silently discarding the supplied document: repositories are unioned by id (on-disk order first, new ids appended in supplied order, first occurrence of an id wins) and the merged document is written to `.clossys/inventory.json`. The both-empty refusal is unchanged.
- Resume with `--inventory` now refuses with "hub already appointed; edit `.clossys/inventory.json`" instead of the misleading "--inventory is only valid when appointing".
