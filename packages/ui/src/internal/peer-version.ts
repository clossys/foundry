 ...
 * so a token-only consumer can install `@vespeneventures/ui` and use
 * `@vespeneventures/ui/tokens` or the CSS subpaths without installing any
 ...
 *     `@vespeneventures/ui/tokens` instead — an explicit, Node-only,
...
export function assertPeerVersion(input: AssertPeerVersionInput): void {
  ...
  if (!outcome.evaluated && outcome.kind === "unparseable-version") {
    ...
    if (!warnedUnparseableVersions.has(warnKey)) {
      ...
      console.warn(
        `[@vespeneventures/ui] Could not verify ${peer}@${foundVersion} against this package's declared range ` +
