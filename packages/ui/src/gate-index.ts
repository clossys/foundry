/**
 * `@vespeneventures/ui/gate` — the public entry point for this package's
 ...
 * `@vespeneventures/ui/gate` directly, never from the root ladder barrel,
 * the same way `ui-token-check` (see `cli.ts`) is reachable only via its
