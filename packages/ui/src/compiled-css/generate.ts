 ...
 * as `@vespeneventures/ui/compiled.css` (see `README.md`'s "Framework-
 ...
 * @vespeneventures/ui · compiled.css
 * ────────────────────────────────────────────────────────────────────
 ...
 *
 * Scope: this file covers \`@vespeneventures/ui/atoms\` only — the
 ...
 *
 * Load this AFTER \`@vespeneventures/ui/tokens.css\` (never instead of it —
...
function makeStylesheetLoader(stylesDir: string) {
  ...
  function resolveTailwindSubpath(subpath: "tailwindcss/theme.css" | "tailwindcss/utilities.css"): string {
    try {
      ...
      throw new Error(
        `generateCompiledCss: could not resolve "${subpath}" — is "tailwindcss" installed? This generation step requires the real tailwindcss package (already a devDependency/optional peer of @vespeneventures/ui) to be present in node_modules. Original error: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
