...
import { mergeUiClasses } from "@vespeneventures/ui/atoms";
import { Faq, FeatureGrid, Hero } from "@vespeneventures/ui/blocks";
import { SiteFooter, SiteHeader } from "@vespeneventures/ui/shell";

/**
 * One entry in {@link MarketingViewProps.features} — deliberately a single
 * text region (`heading` only, no `description`/`icon`), unlike
 * `@vespeneventures/ui/blocks`' own `FeatureGridItem`. This mirrors the
...
export interface MarketingViewProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /** The closing call-to-action band's own heading. Renders as a second `Hero`-shaped section (`headingLevel={2}`) — see `@vespeneventures/ui/blocks`' `Hero` doc comment, "a page can reasonably contain two `Hero`-shaped sections". */
 ...
 *
 * Composed entirely from already-shipped `@vespeneventures/ui` primitives
