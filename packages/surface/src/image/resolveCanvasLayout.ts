 ...
 * see this file's own logic below and `@vespeneventures/surface/core`'s own
 ...
 * bar exists to catch — the same rule `@vespeneventures/surface/core`'s own
 ...
 * render the exact shape of document `@vespeneventures/surface/core` 0.3.0
...
export interface ResolveCanvasLayoutOptions {
  ...
  /** See `@vespeneventures/surface/core`'s own `AssetLookup`. Omit when no binding in the document uses `assetId` — every `assetId` binding is then treated as unresolved, which is always fatal for this pipeline (see this file's own top comment, "Assets get a stricter bar than optional text"). */
