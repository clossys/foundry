#!/usr/bin/env node
/**
 * `surface-media-check` — the CLI for `checkAssetCoverage`. Presentation only:
 ...
 * findings — and pick an exit code. Matches `@vespeneventures/copy`'s
 * `copy-check` and `@vespeneventures/ui/tokens`' `tokens-brand-check` shape
 ...
 * `SlotBinding.assetId` across a set of `@vespeneventures/surface/core`
 * documents). This package has no scanner of its own — unlike
 * `@vespeneventures/copy`'s `copy-check`, which walks real source files, a
...
const USAGE = `Usage: surface-media-check <record-file> <referenced-ids-file> [options]

  record-file           Path to an AssetRecord JSON file (see @vespeneventures/surface/media). Required.
...
function run(): void {
  try {
    ...
    if (error instanceof CliInputError) {
      console.error(`surface-media-check: ${error.message}`);
      console.error(`\n${USAGE}`);
      process.exitCode = 2;
    } else {
      console.error(`surface-media-check: unexpected error: ${error instanceof Error ? (error.stack ?? error.message) : String(error)}`);
      process.exitCode = 2;
    }
  }
}
 ...
 * `@vespeneventures/ui/tokens`' `cli.ts` both use, for the same reason: `npm
