/**
 * Plain TypeScript types for @vespeneventures/surface/media's contract: one
 ...
 *
 * This module is to `@vespeneventures/surface/core`'s visual half exactly what
 ...
 * `CopyRecord`, `SlotBinding.assetId` (added in `@vespeneventures/surface/core`
 ...
 * `@vespeneventures/policy`, `@vespeneventures/ui/tokens`, `@vespeneventures/copy/voice`,
 ...
 * a consumer's job, never this package's. See `@vespeneventures/copy`'s
...
interface AssetEntryBase {
   ...
   * already set one layer up — `@vespeneventures/surface/core`'s own `ImageMeta.alt`
   ...
   * (`"   "`) are rejected the same way `@vespeneventures/surface/core`'s
   ...
   * `StyleBinding`'s token role names in `@vespeneventures/surface/core` are
