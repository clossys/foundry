 ...
 *
 * `resolveCopyId`'S TYPE IS `@vespeneventures/copy`'s REF-BASED
 ...
 * `resolveSurfaceDocument` already does. Only `@vespeneventures/copy`'s
...
export interface RenderStructuredDocumentOptions {
  /** See this file's own top comment, "`resolveCopyId`'s type", for why this is `@vespeneventures/copy`'s ref-based `CopyResolver`, not `surface/web`'s string-keyed one. Omit only for a document with no sections at all — every other document has at least `title` to resolve. */
