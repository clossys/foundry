/**
 * Public types for `@vespeneventures/surface/print`. See
 ...
 * `@vespeneventures/surface/core`'s own README, "The `copyId` seam", for the
 ...
 * contract — see `@vespeneventures/surface/core`'s `types.ts`) has no
...
export interface RenderPrintOptions {
  ...
  /** Brand token overrides, passed straight through to `@vespeneventures/ui/tokens`'s `flattenTokens(overrides)` (via the shared `internal/tokens.ts`). Omit to render with every token at its own default value. */
