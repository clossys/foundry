 ...
 * output, i.e. a role name IS the exact `@vespeneventures/ui/tokens`
  ...
  if (value === undefined) {
    throw new RenderError(
      ...
      `renderPrintDocument: ${ownerDescription}'s style.${field} names token role "${role}", which is not a ` +
        `property in @vespeneventures/ui/tokens' TOKENS registry (checked after flattening — see the shared ` +
