/**
 * `ElementKind` -> real `@vespeneventures/ui/tokens` typography, resolved to
 ...
 * ---------------------------
 * `@vespeneventures/ui/tokens` DOES ship a full typography scale — 13 `text`
 ...
 * -------------------------------------------------------------
 * `StyleBinding` (`@vespeneventures/surface/core`'s frozen contract) carries no
 ...
 *     see `@vespeneventures/surface/core`'s own `types.ts`).
 *
 * Every value in both maps is asserted, in this file's own test, to be a
 * REAL key in `@vespeneventures/ui/tokens`' `TOKENS` registry — see
...
export function resolveElementFontFamily(element: ElementKind, flat: ReadonlyMap<string, string>): string {
  ...
  if (!flat.has(role)) {
    throw new RenderError(
      "unknown-style-role",
      `resolveElementFontFamily: ElementKind ("${element}") default font-family role names token role "${role}", which is not a property in @vespeneventures/ui/tokens' TOKENS registry (checked after flattening) — this is a bug in this file's own mapping, not caller input.`,
    );
  }
