...
import { checkBrandFileCoverage, TOKENS } from "@vespeneventures/ui/tokens";

import { RenderError } from "./errors.js";
import { flattenTokens } from "./tokens.js";

/**
 * `@vespeneventures/ui/tokens`' `checkBrandFileCoverage` and this package's own
 ...
 *
 * This lives HERE, in `@vespeneventures/surface` (which already depends on
 * `@vespeneventures/ui/tokens` — see `package.json`), rather than inside
 * `@vespeneventures/ui/tokens` itself, because the reverse dependency would be
 * circular: `@vespeneventures/ui/tokens` cannot import
 * `@vespeneventures/surface` to test against `flattenTokens` without
 ...
 */
describe("checkBrandFileCoverage (@vespeneventures/ui/tokens) agrees with flattenTokens (this package) on the non-brandable-override rule", () => {
