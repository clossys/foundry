 ...
 * `@vespeneventures/surface/core` 0.3.0 added `SlotBinding.assetId` and
 * `@vespeneventures/surface/media` exists to register what an `assetId` actually
 * points AT. Built once here, rather than reinvented (and re-drifted) five
 * times — the exact reasoning `@vespeneventures/surface/core`'s own
 ...
 *
 * `RenderAsset` IS NOT AN IMPORT OF `@vespeneventures/surface/media`'S `AssetEntry`
 * -----------------------------------------------------------------------------
 ...
 * not import `@vespeneventures/surface/media` to get it. Same "opaque seam,
 * not a code import" reasoning `@vespeneventures/surface/core`'s own
 * `resolve-assets.ts` gives for `AssetLookup` itself: `@vespeneventures/
 * surface` works whether or not `@vespeneventures/surface/media` is even
 ...
 * actually paints and not `@vespeneventures/surface/media`'s own
 ...
 * `@vespeneventures/surface/media`'s own — see `RenderImageAsset`/
 * `RenderVideoAsset` below.
 *
 * ALT IS REQUIRED HERE FOR THE IDENTICAL REASON IT IS REQUIRED ON `AssetEntry`
 * -------------------------------------------------------------------------------
 * `@vespeneventures/surface/media`'s own `AssetEntryBase.alt` doc comment:
 ...
 * -------------------------------------------------------------------------------
 * `@vespeneventures/surface/media`'s `schema.ts` refuses a `VideoAssetEntry`
 ...
 * case `@vespeneventures/surface/media`'s `ImageAssetEntry.src` doc
...
/** One additional resolution/format for a `RenderImageAsset` — see `@vespeneventures/surface/media`'s `ImageSource`, which this mirrors structurally without importing it (see this file's own top comment). */
...
/** One playable `<source>` for a `RenderVideoAsset` — see `@vespeneventures/surface/media`'s `VideoSource`. */
export interface RenderVideoSource {
  src: string;
  mimeType: string;
}

/** One `<track kind="captions">` for a `RenderVideoAsset` — see `@vespeneventures/surface/media`'s `VideoCaption`. */
 ...
 * {@link isRenderVideoAsset} even though `@vespeneventures/surface/media`'s
...
/** Either shape a resolved `assetId` binding can now produce — discriminated by `type`, mirroring `@vespeneventures/surface/media`'s own `AssetEntry` union without importing it (see this file's own top comment). */
 ...
 * discipline `@vespeneventures/surface/media`'s own `schema.ts` holds to.
...
/**
 * What `resolveDocumentAssets` returns — `@vespeneventures/surface/core`'s own
...
export interface RenderAssetResolution {
  ...
  /** `assetId`s `lookup` returned `undefined`/`null` for — see `AssetLookup`'s own doc comment in `@vespeneventures/surface/core`. */
  unresolvedAssetIds: string[];
  /** Slot keys `resolveAssets` could not even attempt to resolve — no source, an ambiguous binding, `lookup` not a function, or `lookup` threw for that slot. See `@vespeneventures/surface/core`'s own `resolveAssets` doc comment. */
  unchecked: string[];
  ...
  /** Slot keys legitimately not this pass's job — their one source of content is `copyId`/`value`, not `assetId`. Never a failure; see `@vespeneventures/surface/core`'s own `resolveAssets`, `deferredToCopy`. */
 ...
 * `@vespeneventures/surface/core` itself.
 *
 ...
 * `@vespeneventures/surface/core`'s own `resolveAssets` — see this package's own
