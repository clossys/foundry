/**
 * `@vespeneventures/ui/tokens`'s `TOKENS` registry, flattened to literal,
...
/**
 * `flattenTokens` — every entry in `@vespeneventures/ui/tokens`' `TOKENS`
...
export function flattenTokens(overrides: Record<string, string> = {}): Map<string, string> {
  ...
  if (unknownSlots.length > 0) {
    throw new RenderError(
      ...
      `flattenTokens: override(s) target token slot(s) that do not exist in @vespeneventures/ui/tokens' TOKENS registry: ${unknownSlots.join(", ")}`,
    );
  }
