/**
 * `liveStateSurface`: a runtime pin, a machine manifest, and a deployment
 * target are the same statement at three altitudes — *this is what should
 * exist; go and see whether it does.* This module is that statement, named
 * once, generalized from two independent instances that arrived at the same
 * shape (a scheduler's declared cadence versus what the host actually holds,
 * and a routine's declared skill versus what the scheduler actually
 * registered) plus a third from a consuming plane: a declared repository
 * standard a script could not read back because the credential it needed was
 * not one the default CI token could be granted.
 *
 * Three things every subject this package reconciles needs to say, and one
 * failure mode every one of them can fall into:
 *
 ...
 * result. Both existing tiers this generalizes already learned that lesson
 * independently; the one addition this module makes over both of them is
 * naming the state neither one named: `declared-but-not-verifiable`. A
 ...
 * instead. `satisfied` is `verified`, `violated` is `drifted`, and
 * `indeterminate` is `could-not-verify`, carrying `declared-but-not-verifiable`
 * as its one declared reason and the named blocker as `detail`.
 *
