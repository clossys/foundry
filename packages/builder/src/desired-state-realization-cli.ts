...
export function readBuilderAssessmentJson(path: string): unknown {
  const resolved = resolve(path);
  if (!existsSync(resolved)) throw new BuilderCliInputError(`assessment file "${path}" does not exist`);
  try {
    if (!statSync(resolved).isFile()) throw new BuilderCliInputError(`assessment file "${path}" is not a file`);
    return JSON.parse(readFileSync(resolved, "utf8"));
  } catch (cause) {
    if (cause instanceof BuilderCliInputError) throw cause;
    throw new BuilderCliInputError(`assessment file "${path}" is unreadable JSON: ${cause instanceof Error ? cause.message : String(cause)}`);
  }
}
...
export function main(argv: readonly string[]): number {
  ...
  if (argv.length !== 1) throw new BuilderCliInputError("exactly one assessment.json file is required");
  const report = assessDesiredStateRealizationRate(readBuilderAssessmentJson(argv[0] as string));
...
function run(): void {
  try {
    ...
    console.error(`builder-check: ${cause instanceof Error ? cause.message : String(cause)}`);
    process.exitCode = 2;
  }
}
