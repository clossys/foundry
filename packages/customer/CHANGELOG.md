...
- `skill/SKILL.md` now describes the bounded, independent keep procedure
  ...
  text-only reasoning about a screenshot's existence), and a pointer to
  `packages/designer/PRE-AUTH-QUALITY.md`'s "Bounded taste pass" section
  for the exact round and wall-clock caps, so the numbers stay declared in
  one place instead of drifting between packages.
