...
describe("resolveInstalledPeerVersion", () => {
  it("resolves a real installed package's version from disk", () => {
    // "svix" is this package's own peer AND devDependency, so it is
    // guaranteed to be on disk in this repository's own workspace.
    const version = resolveInstalledPeerVersion("svix", import.meta.url);
    expect(version).toMatch(/^\d+\.\d+\.\d+$/);
  });

  it("resolves a peer whose own exports map does not expose ./package.json (the @clerk/nextjs / resend case)", () => {
    const version = resolveInstalledPeerVersion("@clerk/nextjs", import.meta.url);
    expect(version).toMatch(/^\d+\.\d+\.\d+$/);
  });
