 ...
 * (see that file's own header for why: a `"use client"` module like
 * `client.tsx` must never import from a file carrying these imports, even
 * a named export it never calls, since a browser bundle cannot resolve
 * `node:module`/`node:fs` at all). Only `verify.ts` and
 * `server-routes.tsx` — both unambiguously Node-context, never reachable
 * from `client.tsx`'s own browser-bundled graph — import this file.
 */
