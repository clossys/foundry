import type { ExternalMembershipEvent } from "../../membership.js";

/** The Clerk event names understood by this provider adapter. */
...
/** A lifecycle operation, normalized independently of event-name spelling. */
...
/** The only fields retained for a user lifecycle event. */
...
/** The only fields retained for an organization lifecycle event. */
...
/** A provider membership event ready for the reconciliation boundary. */
...
export type ClerkResolvedMembershipEvent = ClerkMembershipEvent & {
  localIds: { userId: string; tenantId: string };
};
...
/** The result of attempting to map an event. */
...
/** Maps a provider role to a product-owned role without imposing a vocabulary. */
