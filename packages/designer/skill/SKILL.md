...
3. **Customer inhabit** — independent `@clossys-customer` session. First-person keep of the named Audience, not a checklist. That person can also be asked for lived feedback on any topic, comparison from their consideration set, what it would take to start or to refer, whether it is worth what it costs them, and what would make them leave. This role does not attend as the judge.
...
Done is exceptional (5) as defined in `PRE-AUTH-QUALITY.md` in this package. `designer-hero-css-check`, `designer-fold-check`, and `writer-check --live` prove 3 only — never call 3 done or world class. A 5 keep is independent QA inhabited as the named Strategist Audience, comprehensive, not a visual score. This role does not author keep-review evidence and does not inhabit the persona.
