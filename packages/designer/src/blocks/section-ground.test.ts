...
describe("SECTION_GROUND_CLASSES", () => {
  ...
  it("keeps readiness display tokens separate from the off-axis mark on every ground", () => {
    for (const ground of Object.values(SECTION_GROUND_CLASSES)) {
      ...
      expect(ground.offAxis).toContain("bg-ink-muted");
    }
    expect(SECTION_GROUND_CLASSES.base.status.success).not.toContain("ring-");
    expect(SECTION_GROUND_CLASSES.base.offAxis).not.toContain("ring-");
    expect(SECTION_GROUND_CLASSES.sunken.status.success).toContain("ring-ink-primary");
    expect(SECTION_GROUND_CLASSES.sunken.offAxis).toContain("ring-ink-primary");
    expect(SECTION_GROUND_CLASSES.inverse.status.success).toContain("ring-ink-on-inverse");
    expect(SECTION_GROUND_CLASSES.inverse.offAxis).toContain("ring-ink-on-inverse");
    ...
    expect(checkedPairs.has("ink-muted/surface-base")).toBe(true);
