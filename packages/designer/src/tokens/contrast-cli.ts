#!/usr/bin/env node
/**
 * `designer-contrast-check` — the CLI for `checkTokenContrast`, mirroring
 * `designer-token-check`'s shape (`../cli.ts`) as closely as this gate's own
 ...
 *
 * UNLIKE `designer-token-check`, there is no source-directory walk: this gate
 ...
 * instance), the same "swap the registry" escape hatch `designer-token-check`'s
 ...
 * repository uses (`designer-token-check`, `designer-brand-check`,
 ...
 *       mirroring `designer-token-check`'s own `unchecked` handling — at least
 ...
 *       either, the same discipline `designer-token-check` holds `TokenGateResult
 *       .unchecked` to.
 */
...
const USAGE = `Usage: designer-contrast-check [tokens-css-file] [options]
...
function printThemeReport(themeLabel: string, result: ContrastGateResult): void {
  ...
  // designer-token-check's own unchecked report — it means the same thing here:
...
function run(): void {
  try {
    ...
    if (error instanceof CliInputError) {
      console.error(`designer-contrast-check: ${error.message}`);
      console.error(`\n${USAGE}`);
      process.exitCode = 2;
    } else {
      console.error(`designer-contrast-check: unexpected error: ${error instanceof Error ? (error.stack ?? error.message) : String(error)}`);
      process.exitCode = 2;
    }
  }
}
