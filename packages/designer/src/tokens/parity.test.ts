 ...
 * `src/tokens.ts`, both claim to describe the same 159 tokens. Nothing
...
describe("tokens.css <-> src/tokens.ts parity", () => {
  ...
  it("declares exactly 159 tokens (the curated set this package promises)", () => {
    expect(declarations.size).toBe(159);
    expect(Object.keys(TOKENS).length).toBe(159);
  });
});
