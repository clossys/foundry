...
export class ClerkWebhookSignatureError extends Error {
  ...
  constructor(
    readonly code: ClerkWebhookSignatureErrorCode,
    readonly missingHeaders: readonly string[] = [],
  ) {
    super(
      code === "signature-headers-missing"
        ? "Clerk webhook verification requires the Svix signature headers."
...
function readRequiredHeaders(headers: ClerkWebhookHeaders): Record<(typeof requiredHeaderNames)[number], string> {
  ...
  if (missingHeaders.length > 0) {
    throw new ClerkWebhookSignatureError("signature-headers-missing", missingHeaders);
  }
...
/**
 * Verifies the exact bytes received from Clerk through Svix. The returned
 * value intentionally excludes the raw body, headers, and signing secret.
 */
