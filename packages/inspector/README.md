...
| `ReviewEvidenceOptions` | Type. The commit under test, and whether review presence is required independently of the policy's verdict rules. On a merge-queue run, `mergeGroup` names the group commit and whether it contains the commit under test (the queued pull request's head); anything but a proven `true` is indeterminate. |
| `ReviewEvidenceMergeGroup` | Type. The merge-queue group commit and the caller's ancestry answer for the commit under test. |
