 ...
 * `@vespeneventures/tokens` or `@vespeneventures/voice`. This is the exact
 * discipline `Market.factRefs`/`Audience.factRefs`/`Claim.factRef` already
 * use throughout this package and `@vespeneventures/voice` — a string
 ...
 * t.brandable).map(t => t.property)` from `@vespeneventures/tokens`
 ...
 * -----------------------------------------------------------------------
 * `@vespeneventures/tokens`' `brand-coverage.test.ts` checks two
...
export interface BrandDerivation {
  ...
  tokenSlots: string[];
  /** Voice rule ids (a consumer's own `@vespeneventures/voice` glossary/claim ids). Referenced by NAME ONLY — see this file's header comment. */
  voiceRules: string[];
 ...
 * `@vespeneventures/tokens` to look it up) and for why the two degenerate
