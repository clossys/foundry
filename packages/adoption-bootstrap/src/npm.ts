...
/** Validates npm's root manifest and lock-v3 package entry without accepting a range or a caller-selected location. */
export function validateNpmIdentity(manifest: unknown, lock: unknown, expected: ExactPackage): string[] {
  ...
  if (!declaredVersion(root, expected)) findings.push(`package-lock root does not declare ${expected.name} at exact ${expected.version}`);
