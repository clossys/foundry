 ...
 * `src/tokens.ts`, both claim to describe the same 128 tokens. Nothing
...
describe("tokens.css <-> src/tokens.ts parity", () => {
  ...
  it("declares exactly 128 tokens (the curated set this package promises)", () => {
    expect(declarations.size).toBe(128);
    expect(Object.keys(TOKENS).length).toBe(128);
  });
});
