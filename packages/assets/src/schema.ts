 ...
 *     all — the exact gap `@vespeneventures/compose`'s `validate.ts` had to
...
/** A short, readable description of an arbitrary value for an error message — mirrors `@vespeneventures/compose`'s `validate.ts`'s `describe` and `@vespeneventures/strategy`'s `validation.ts`'s `describeValue`. */
...
function validateAssetEntryShape(value: unknown, path: string): AssetFinding[] {
  ...
  // @vespeneventures/compose's validate.ts, both of which document why: a
