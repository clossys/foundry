#!/usr/bin/env node
/**
 * `assets-check` — the CLI for `checkAssetCoverage`. Presentation only:
 ...
 * `copy-check` and `@vespeneventures/tokens`' `tokens-brand-check` shape
 ...
 * `SlotBinding.assetId` across a set of `@vespeneventures/compose`
...
const USAGE = `Usage: assets-check <record-file> <referenced-ids-file> [options]

  record-file           Path to an AssetRecord JSON file (see @vespeneventures/assets's README). Required.
...
function run(): void {
  try {
    ...
    if (error instanceof CliInputError) {
      console.error(`assets-check: ${error.message}`);
      console.error(`\n${USAGE}`);
      process.exitCode = 2;
    } else {
      console.error(`assets-check: unexpected error: ${error instanceof Error ? (error.stack ?? error.message) : String(error)}`);
      process.exitCode = 2;
    }
  }
}
 ...
 * `@vespeneventures/tokens`' `cli.ts` both use, for the same reason: `npm
