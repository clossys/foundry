...
`@vespeneventures/render/web`, `/image`, or `/slides` (and, later,
`/email`, `/print`). `@vespeneventures/compose` and
...
all three are required to use `./web`. `./image` and `./slides` need NONE
of those three peers — they emit SVG as a plain string, with zero new
