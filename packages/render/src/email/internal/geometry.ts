 ...
 * positions on a two-dimensional canvas (see `@vespeneventures/compose`'s
 ...
 * -----------------------------------------------------
 * `@vespeneventures/compose`'s own `types.ts` says a `layout` must be
...
export const SYNTHETIC_FRAME: Frame = { x: 0, y: 0, w: 1, h: 1 };

/** The `ElementKind` a synthetic slot is given — no real template supplied one, and `"body"` is the closest thing this vocabulary has to "plain flowing text with no special role." */
export const SYNTHETIC_ELEMENT: ElementKind = "body";

/**
 * Builds a `LayoutSpec` with exactly one `SlotSpec` per DISTINCT
 * `binding.slot` in `bindings`, in first-occurrence order, every slot using
 * {@link SYNTHETIC_FRAME} and {@link SYNTHETIC_ELEMENT}, `required: false`.
 ...
 */
export function buildSyntheticLayout(bindings: ReadonlyArray<{ slot: unknown }>): { slots: SlotSpec[] } {
  ...
  for (const binding of bindings) {
    ...
    seen.add(key);
    slots.push({ key, element: SYNTHETIC_ELEMENT, frame: SYNTHETIC_FRAME, required: false });
  }
  return { slots };
}
