 ...
 *
 * This lives HERE, in `@vespeneventures/render` (which already depends on
 * `@vespeneventures/tokens` — see `package.json`), rather than inside
 * `@vespeneventures/tokens` itself, because the reverse dependency would be
 * circular: `@vespeneventures/tokens` cannot import
 * `@vespeneventures/render` to test against `flattenTokens` without
