 ...
 * gets compared to content, because `@clossys/controller/policy` already owns
 ...
 * expectedDigest }` — and calls `@clossys/controller/policy`'s own `verifyBinding`
 ...
 * concern, the same way `@clossys/controller/policy` never reads a file itself.
 */
