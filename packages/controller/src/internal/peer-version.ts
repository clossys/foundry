 ...
 * install-time signal in either direction" problem. `typescript` is
 * declared `peerDependenciesMeta: { optional: true }` (see package.json)
 * so a consumer can install this package without the compiler — correct,
 * since most of this package's gates never touch it. But an ABSENT or
 * OUT-OF-RANGE `typescript` produced no signal of any kind until now: a
 * consumer on an incompatible compiler learned about it from whatever
 * `secret-gates.ts` happened to crash on deep inside the TypeScript
 * compiler API, with nothing naming a version range as the cause. See
 * `secret-gates.ts`'s own guard call for where this is wired in.
 *
 ...
 * package's own declared peer range (`"typescript": "~6.0.0"`) is a `~`
 * range the ported logic already covers, but `assertPeerVersion` is
 * shared, byte-for-byte identical, across every package in this PR — and
 ...
 *
 * DELIBERATELY PURE — NO `node:*` IMPORTS IN THIS FILE. `secret-gates.ts`
 * reads `typescript`'s own exported `ts.version` rather than a filesystem
...
export interface AssertPeerVersionInput {
  /** The optional peer's package name, e.g. `"typescript"`. */
  peer: string;
