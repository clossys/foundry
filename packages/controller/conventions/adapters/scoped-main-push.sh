...
if [[ -z "$prefix" ]]; then
  ...
  echo "scoped-main-push-hook: AGENT_BRANCH_PREFIX is unset; refusing to run unconfigured." >&2
  exit 0
fi
...
if [[ "$canonical_workspace" == true && -n "$created_branch" && "$created_branch" != "$prefix"/* ]]; then
...
# Cover the direct-push forms this hook exists to block.
