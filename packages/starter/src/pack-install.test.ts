...
describe("packed installed activation canaries", () => {
  it("isolates real npm and pnpm consumers from hostile publish settings and preserves 0/1/2 outcomes", async () => {
    ...
    await withHostilePublishEnvironment(fixtureRoot, async () => {
      ...
      writeJson(join(targetPackage, "package.json"), { name: "@fixture/starter-target", version: "1.0.0", private: true, type: "module", bin: { "fixture-target-check": "./bin/check.js" }, peerDependencies: { "@clossys/advisor": "0.1.5" }, files: ["bin"] });
      ...
      const starter = pack(join(repoRoot, "packages/starter"), packed, "@clossys/starter", "0.1.4", fixtureRoot);
      const advisor = pack(join(repoRoot, "packages/advisor"), packed, "@clossys/advisor", "0.1.5", fixtureRoot);
