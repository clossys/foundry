...
import { IntegratorCliInputError, isDirectInvocation, main } from "./package-currency-rate-cli.js";
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "integrator-check-"));
...
beforeAll(() => {
  ...
  if (built.status !== 0) throw new Error(`Integrator build failed: ${built.stderr || built.stdout}`);

  const workDir = mkdtempSync(join(tmpdir(), "integrator-check-bin-"));
  ...
  for (const file of ["package-currency-rate-cli.js", "package-currency-rate.js"]) {
    cpSync(join(packageRoot, "dist", file), join(installedDistDir, file));
  }
  const installedCliPath = join(installedDistDir, "package-currency-rate-cli.js");
  chmodSync(installedCliPath, 0o755);
  binPath = join(dotBin, "integrator-check");
...
describe("integrator-check", () => {
  it("maps satisfied, violated, and indeterminate reports to 0, 1, and 2", () => {
    ...
    expect(main([write("empty.json", { asOf: "2026-09-18T12:00:00Z", declaredPackages: [], observations: [] })])).toBe(2);
  });

  it("uses exit-2 input semantics and documents the tri-state contract", () => {
    expect(() => main([])).toThrow(IntegratorCliInputError);
    expect(() => main([join(root, "missing.json")])).toThrow(IntegratorCliInputError);
  ...
  it("recognizes invocation through an installed-style POSIX bin symlink", () => {
    const sourceUrl = new URL("./package-currency-rate-cli.ts", import.meta.url);
    const linked = join(root, "integrator-check");
