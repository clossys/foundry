 ...
 * owns that join exactly once. Neither caller re-implements it.
 *
 * `check-public-npm-provenance.mjs` (this repository's own tooling, not
 * shipped in this package) imports this file directly by relative path,
 * unbuilt. That only works because this module has zero dependencies and
 * uses no TypeScript syntax beyond type annotations and interfaces (no
 * enums, no namespaces, no parameter-property constructors), so Node's
 * native type-stripping (stable and unflagged since Node 23.6, see
 * https://nodejs.org/api/typescript.html) can run it as-is. That keeps the
 * publish workflow's anonymous post-publish verification job exactly as
 * cheap as it was before this package existed: checkout, then run two
 * scripts, no install and no build. Do not add TypeScript syntax here that
 * type-stripping cannot erase.
 */
