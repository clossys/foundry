...
describe("installed repository-check packages", () => {
  it("keep imports hostile-clean and execute through npm-created bin symlinks", { timeout: 60_000 }, async () => {
    ...
    const policyTarball = await pack("policy", tarballs);
    ...
    await installTarballs(governanceConsumer, [policyTarball, governanceTarball]);
    ...
    await installTarballs(facadeConsumer, [policyTarball, governanceTarball, repositoryTarball]);
    await execFile("node", ["--input-type=module", "--eval", hostileImport,
