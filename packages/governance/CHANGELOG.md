...
- The root entry (`import "@vespeneventures/governance"`) no longer
  ...
  compiler and its `node:fs`/`node:child_process` usage; a consumer who
  deliberately imports the public `@vespeneventures/governance/gates`
  subpath still gets everything, unchanged, secret-gates included.
