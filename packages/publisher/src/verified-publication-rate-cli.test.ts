...
import { PublisherRateCliInputError, isDirectInvocation, main } from "./verified-publication-rate-cli.js";
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "publisher-rate-check-"));
...
beforeAll(() => {
  ...
  if (built.status !== 0) throw new Error(`Publisher build failed: ${built.stderr || built.stdout}`);

  const workDir = mkdtempSync(join(tmpdir(), "publisher-rate-check-bin-"));
  ...
  for (const file of ["verified-publication-rate-cli.js", "verified-publication-rate.js"]) {
    cpSync(join(packageRoot, "dist", file), join(installedDistDir, file));
  }
  const installedCliPath = join(installedDistDir, "verified-publication-rate-cli.js");
  chmodSync(installedCliPath, 0o755);
  binPath = join(dotBin, "publisher-rate-check");
...
describe("publisher-rate-check", () => {
  ...
  it("uses exit-2 input semantics and documents the tri-state contract", () => {
    expect(() => main([])).toThrow(PublisherRateCliInputError);
    expect(() => main([join(root, "missing.json")])).toThrow(PublisherRateCliInputError);
  ...
  it("recognizes invocation through an installed-style POSIX bin symlink", () => {
    const sourceUrl = new URL("./verified-publication-rate-cli.ts", import.meta.url);
    const linked = join(root, "publisher-rate-check");
    symlinkSync(fileURLToPath(sourceUrl), linked);
    expect(isDirectInvocation(sourceUrl.href, linked)).toBe(true);
  });
});

describe("publisher-rate-check, invoked through a node_modules/.bin-shaped symlink", () => {
  it("exits 0 with a satisfied report", () => {
    const result = runBin([write("satisfied.json", satisfied())]);
    expect(result.status).toBe(0);
    expect(JSON.parse(result.stdout)).toMatchObject({ state: "satisfied", metric: "verified publication rate", rate: 1 });
  });
