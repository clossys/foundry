 ...
 * (see that file's own header for why: `renderWebDocument.ts` — reachable
 * from a browser bundle as easily as from a server one — must never
 * import from a file carrying these imports, even a named export it
 * never calls, since a browser bundle cannot resolve
 * `node:module`/`node:fs` at all). Nothing in this package's own source
 * currently calls this function (`renderWebDocument.ts` reads `react`'s
 * own exported `version` instead — see its own comment); it ships for
 * parity with the other packages' internal peer-version guard and for any
 * future Node-only guard site this package adds.
 */
