...
describe("packed Publisher web React-server boundary", () => {
  ...
  it("rejects a packed Designer 0.2.5 graph that the former lower bound accepted before SectionedView's missing blocks fail at import", () => {
    ...
    // does carry the current one. Tracks Designer's floor — a Designer bump
    // that raises the floor again updates this line too, the same way
    // `expectedVersions` in scripts/run-candidate-qualification.test.mjs
    // tracks every package.
    expect(legacyManifest.dependencies["@clossys/designer"]).toBe("^0.4.12");
