...
| [`@vespeneventures/governance`](packages/governance) | A pure governance evaluation primitive for a GitHub App adapter: verifies webhook HMACs, evaluates exact pull-request head/base facts plus caller-injected tenant policy, and emits a completed check-output payload. It does not call GitHub, load tenant configuration, retain secrets, or decide merges. |
