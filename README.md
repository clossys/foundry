...
| [`@vespeneventures/auth-agent`](packages/auth-agent) | Dependency-free authorization guards for time-bounded software-agent contexts: lifecycle evaluation, tool-capability checks, monetary limits, and typed denial reasons. Performs no I/O and leaves identity verification to the caller. |
...
| [`@vespeneventures/auth-web-clerk`](packages/auth-web-clerk) | Minimal Next.js 16 and Clerk 7 adapter with separate client/server exports: provider mount, hosted sign-in block, error normalization, explicit proxy policy, strict redirects, sign-in pages, and session-revoking sign-out routes. |
