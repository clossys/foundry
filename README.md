...
| [`@clossys/launcher`](packages/launcher) | Foundry Launcher is non-role executable tooling for step 0: create, resume, or appoint one GitHub repository as the account workspace hub. It copies an in-package skeleton or writes a hub marker beside existing files, pins Advisor as a development dependency, and does not dump the catalogue. It is not a role, not a connector, and not Starter's protected-base `decide` gate. |
| [`@clossys/advisor`](packages/advisor) | The Advisor role — does each active engagement have a current, evidence-backed, authority-bound next decision or action? Provider-neutral sponsor dialogue, engagement-state reconciliation, offering-fit and readiness assessment, blocker and initiative-collision detection, bounded first-wave recommendations including install, remove, and relocate, caller-supplied hub-placement evidence, required next actions, reassessment triggers, and an execution gate that never treats unresolved pre-work as passive HOLD. |
...
| [`@clossys/customer`](packages/customer) | The customer role — if I am the person this was made for, do I keep this? A first-person synthetic inhabit of the named Strategist audience, on speed dial for keep plus lived feedback, compare, refer, churn, adopt, and worth testimony. Not a reviewer, not hired QA, not competitive-intel, and not Influencer yield. |
