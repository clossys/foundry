...
| [`@vespeneventures/policy`](packages/policy) | Deprecated compatibility entry point for `@vespeneventures/controller`'s policy subpath; retained while consumers migrate. |
...
| [`@vespeneventures/governance`](packages/governance) | Deprecated compatibility entry point for `@vespeneventures/controller`; retains every subpath and the `foundry-governance`/`foundry-check`/`repository-check`/`review-check` commands while consumers migrate. |
...
| [`@vespeneventures/verify-standards`](packages/verify-standards) | One repository-standards gate a consumer's own thin workflow invokes: a secret-scan attempt, a change's task record, its review evidence, and drift between a declared standard and the live state enforcing it. Every check is a pure function of caller-collected observations, reports the `satisfied`/`violated`/`indeterminate` ternary rather than a boolean, and folds into a `0`/`1`/`2` exit contract with no flag that can turn a could-not-evaluate into a pass. Also holds itself to a minimum-safe-version floor, so a caller running or still permitting a pre-floor build is told so as a failure rather than a warning. Two runtime dependencies (`@vespeneventures/governance` and `@vespeneventures/policy`, both pinned with tilde ranges). |
...
| [`@vespeneventures/secret-scan`](packages/secret-scan) | Verified gitleaks binary download and secret-scan gate utilities. Pure, provider-agnostic mechanism for CI secret scanning with SHA-256 verified binary downloads. Zero runtime dependencies (optional adm-zip/tar for archive extraction). |
