...
function checkTranscript(a, t) {
  closed(a, t, ["schema", "version", "candidate", "archetype", "tarball", "peerInstall", "consumer", "coverage", "observations", "dimensions", "restoration", "mismatches", "ok", "canonicalSha256"], "transcript");
  if (t?.schema !== "foundry-candidate-qualification-transcript-v1" || t?.version !== 1 || !object(t.candidate) || !NAME.test(t.candidate.name) || !VERSION.test(t.candidate.version) || !ARCHETYPES.includes(t.archetype) || t.ok !== true || !SHA256.test(t.canonicalSha256)) fail(a, "transcript", "generic transcript");
...
export function validateCandidateQualification(r, { mode = "offline", expected, freshTranscript } = {}) {
  ...
  if (freshTranscript && JSON.stringify(stable(r.transcript)) !== JSON.stringify(stable(freshTranscript))) fail(a, "fresh-transcript", "fresh transcript");
