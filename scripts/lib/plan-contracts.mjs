 ...
 * #1178), which only it computes; see LAUNCHER_CONTRACT_FILES.
 *
 ...
 *   followed by the repository change-set and apply-bundle contracts
 *   (issue #1178), under LAUNCHER_DATA_BANNER. It also packs the scope
 *   and registry from `package-scope.json`, because its snapshot step
 *   fetches from that registry, refuses a package outside that scope, and
 *   validates the snapshot it writes against that contract.
 *
 ...
 * `advisor-plan.json`'s definitions by file name).
 */
export const LAUNCHER_CONTRACT_FILES = [...ADVISOR_PLAN_CONTRACT_FILES, "repository-change-set.json", "apply-bundle.json"];
...
// registry snapshot contract, and the repository change-set and
// apply-bundle contracts, in this package's source repository (not shipped
// in this package). The plan, brief and inventory contracts here are the
// same data every package that validates a plan, a brief or an inventory
// packs.
`;
...
export function renderPlanContractsModule(repoRoot, files = PLAN_CONTRACT_FILES) {
  ...
  const what =
    files === ADVISOR_PLAN_CONTRACT_FILES ? "The shared plan, brief and inventory contracts, and the registry snapshot contract"
      : files === LAUNCHER_CONTRACT_FILES ? "The shared plan, brief, inventory and registry snapshot contracts, and the change-set and bundle contracts"
