 ...
 *   then the repository change-set and apply-bundle contracts (issue #1178),
 *   under LAUNCHER_DATA_BANNER. It
...
// contract, and the repository change-set and apply-bundle contracts, in this
// package's source repository (not shipped in this package). The plan and brief contracts here are the
...
export function renderPlanContractsModule(repoRoot, files = PLAN_CONTRACT_FILES) {
  ...
  const what =
    files === ADVISOR_PLAN_CONTRACT_FILES ? "The shared plan and brief contracts, and the registry snapshot contract"
      : files === LAUNCHER_CONTRACT_FILES ? "The shared plan, brief and registry snapshot contracts, and the change-set and bundle contracts"
