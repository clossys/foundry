 ...
 * computes, and the registry snapshot contract, which its snapshot step
 * writes; see LAUNCHER_CONTRACT_FILES.
 *
 ...
 *   followed by the repository change-set and apply-bundle contracts and
 *   `registry-snapshot.json` (issue #1178), under LAUNCHER_DATA_BANNER. It
 ...
 * data in both packages; the two modules differ only in their header and in
 * the entries each package adds after those three.
 *
 ...
 * definitions by file name), then the registry snapshot contract, which
 * Launcher's snapshot step validates a snapshot against before writing it.
 */
export const LAUNCHER_CONTRACT_FILES = [...PLAN_CONTRACT_FILES, "repository-change-set.json", "apply-bundle.json", "registry-snapshot.json"];
...
// Do not edit by hand -- edits are overwritten on the next \`npm run build\`.
// Source of truth: the shared plan and brief contracts, the repository
// change-set and apply-bundle contracts, and the registry snapshot contract,
// in this package's source repository (not shipped in this package). The plan and brief contracts here are the
...
export function renderPlanContractsModule(repoRoot, files = PLAN_CONTRACT_FILES) {
  ...
  const what =
    files === ADVISOR_PLAN_CONTRACT_FILES ? "The shared plan and brief contracts, and the registry snapshot contract"
      : files === LAUNCHER_CONTRACT_FILES ? "The shared plan, brief, change-set, bundle and registry snapshot contracts"
