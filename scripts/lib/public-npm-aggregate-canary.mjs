...
function recordSet(directory, path) {
  const match = new RegExp(`^${directory}/(baseline|oidc-successor)-[a-f0-9]{64}\\.json$`).exec(path ?? "");
  return match?.[1] ?? null;
}

/**
 * Enforce the singular closure/transcript authority per frozen set. The
 * optional reader is called only with an exact Git-derived path in one of the
 * closed namespaces; callers should return parsed committed bytes.
 */
export function validateAggregateRecordSets({ closureRecords = {}, transcriptRecords = {}, readTranscript = null } = {}) {
  const findings = [];
  const pathsFor = (records, directory, set, field) => (Array.isArray(records?.[field]) ? records[field] : [])
    .filter((path) => recordSet(directory, path) === set);
  for (const set of aggregateRecordSets) {
    const introducedClosures = pathsFor(closureRecords, AGGREGATE_CLOSURE_DIRECTORY, set, "introduced");
    const currentClosures = pathsFor(closureRecords, AGGREGATE_CLOSURE_DIRECTORY, set, "current");
    const introducedTranscripts = pathsFor(transcriptRecords, AGGREGATE_TRANSCRIPT_DIRECTORY, set, "introduced");
    const currentTranscripts = pathsFor(transcriptRecords, AGGREGATE_TRANSCRIPT_DIRECTORY, set, "current");
    if (introducedClosures.length > 1) finding(findings, "closure-singularity", `${set} has more than one introduced immutable closure record`);
    if (currentClosures.length > 1) finding(findings, "closure-singularity", `${set} has more than one current immutable closure record`);
    if (introducedTranscripts.length > 1) finding(findings, "transcript-singularity", `${set} has more than one introduced immutable transcript record`);
    if (currentTranscripts.length > 1) finding(findings, "transcript-singularity", `${set} has more than one current immutable transcript record`);
    if (currentTranscripts.length === 0) continue;
    if (currentClosures.length !== 1) {
      finding(findings, "transcript-closure", `${set} transcript requires exactly one current immutable closure record`);
      continue;
    }
    if (!readTranscript) {
      finding(findings, "transcript-closure", `${set} transcript closure binding could not be read`);
      continue;
    }
    for (const path of currentTranscripts) {
      let transcript;
      try { transcript = readTranscript(path); }
      catch { finding(findings, "transcript-closure", `${path} is not readable committed transcript JSON`); continue; }
...
export function validateSatisfiedAggregateTranscript(transcript, { plan, closure, expectedRepositoryRedirects = null, qualificationContracts = null, path = AGGREGATE_CANARY_PATH } = {}) {
  const findings = [];
