...
test("aggregate record sets permit no closure or transcript before completion", () => {
  assert.deepEqual(validateAggregateRecordSets({ closureRecords: { introduced: [], current: [] }, transcriptRecords: { introduced: [], current: [] } }), []);
});

test("aggregate record sets reject hostile duplicate baseline and oidc authorities", () => {
  const closures = ["a", "b", "c", "d"].map((digest, index) => `${index < 2 ? "baseline" : "oidc-successor"}-${digest.repeat(64)}`);
  const transcripts = closures.map((name) => name);
  const closureRecords = { introduced: closures.map((name) => `governance/public-npm-aggregate-closures/${name}.json`), current: closures.map((name) => `governance/public-npm-aggregate-closures/${name}.json`) };
  const transcriptRecords = { introduced: transcripts.map((name) => `governance/public-npm-aggregate-transcripts/${name}.json`), current: transcripts.map((name) => `governance/public-npm-aggregate-transcripts/${name}.json`) };
  const findings = validateAggregateRecordSets({ closureRecords, transcriptRecords });
  assert.equal(findings.filter((item) => item.rule === "closure-singularity").length, 4);
  assert.equal(findings.filter((item) => item.rule === "transcript-singularity").length, 4);
});

test("aggregate record sets require a sole closure before reading and binding a transcript", () => {
  const closurePath = `governance/public-npm-aggregate-closures/baseline-${"a".repeat(64)}.json`;
  const transcriptPath = `governance/public-npm-aggregate-transcripts/baseline-${"b".repeat(64)}.json`;
  let reads = 0;
  const noClosure = validateAggregateRecordSets({ closureRecords: { introduced: [], current: [] }, transcriptRecords: { introduced: [transcriptPath], current: [transcriptPath] }, readTranscript: () => { reads += 1; return {}; } });
  assert.ok(noClosure.some((item) => item.rule === "transcript-closure"));
  assert.equal(reads, 0);
  const mismatch = validateAggregateRecordSets({ closureRecords: { introduced: [closurePath], current: [closurePath] }, transcriptRecords: { introduced: [transcriptPath], current: [transcriptPath] }, readTranscript: (path) => { assert.equal(path, transcriptPath); return { set: "baseline", plan: { closurePath: `governance/public-npm-aggregate-closures/oidc-successor-${"c".repeat(64)}.json` } }; } });
  assert.ok(mismatch.some((item) => item.rule === "transcript-closure"));
});

test("aggregate record sets never pass traversal paths to the transcript reader", () => {
  let reads = 0;
  const findings = validateAggregateRecordSets({ closureRecords: { introduced: [], current: [] }, transcriptRecords: { introduced: ["governance/public-npm-aggregate-transcripts/../outside.json"], current: [] }, readTranscript: () => { reads += 1; return {}; } });
