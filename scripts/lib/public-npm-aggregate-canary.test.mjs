...
function satisfiedFixture() {
  ...
  const transcript = {
    ...
    consumer: { manifestSha256: sha, lockfileSha256: sha, treeSha256: sha, controller: "@clossys/controller@0.8.23", singularController: true, identities: resolved.packages.map((entry) => `${entry.name}@${entry.version}`), runtimeResolutions: [
      ["@clossys/publisher", "@clossys/controller", "~0.8.0"], ["@clossys/publisher", "@clossys/designer", "^0.2.4"], ["@clossys/publisher", "@clossys/writer", "^0.3.0"],
      ["@clossys/builder", "@clossys/controller", "~0.8.0"], ["@clossys/inspector", "@clossys/controller", "~0.8.0"],
...
test("aggregate record closes both frozen 19-package version sets without claiming pending publication", () => {
  ...
  assert.deepEqual(record.sets.flatMap((set) => set.packages).find((entry) => entry.packageKey === "publisher" && entry.version === "0.1.10"), { packageKey: "publisher", name: "@clossys/publisher", version: "0.1.10" });
  assert.deepEqual(record.sets.flatMap((set) => set.packages).find((entry) => entry.packageKey === "publisher" && entry.version === "0.1.11"), { packageKey: "publisher", name: "@clossys/publisher", version: "0.1.11" });
});
...
test("aggregate record fails closed on ordering and non-identity plan rows", () => {
  ...
  for (const mutate of [
    (copy) => { copy.peerResolution.disposition[0].resolved = "1.0.0"; },
    (copy) => { copy.peerResolution.disposition.push(structuredClone(copy.peerResolution.disposition[0])); },
    (copy) => { copy.peerResolution.disposition[0].requested.push("18.3.1"); },
    (copy) => { copy.optionalPeerMatrix.find((row) => row.peers.length > 0).peers[0].outcomes = {}; },
    (copy) => { const row = copy.optionalPeerMatrix.find((item) => item.peers.length > 0); row.peers[0].outcomes["@clossys/writer"] = "imports"; },
    (copy) => { const row = copy.optionalPeerMatrix.find((item) => item.peers.length > 0); row.peers[0].peer = "unreviewed-peer"; },
  ]) {
...
test("optional-peer omission never restores through a replaced consumer, node_modules, or backup root", async () => {
  for (const mode of ["consumer", "node-modules", "backup"]) {
    const temporary = await mkdtemp(join(tmpdir(), "foundry-aggregate-optional-hostile-"));
    const outside = await mkdtemp(join(tmpdir(), "foundry-aggregate-optional-outside-"));
    const peerRoot = join(temporary, "node_modules", "fixture-peer");
    const candidateRoot = join(temporary, "node_modules", "fixture-candidate");
    try {
      await mkdir(peerRoot, { recursive: true });
      ...
      await writeFile(join(temporary, "package-lock.json"), '{"lockfileVersion":3}\n');
      await writeFile(join(peerRoot, "package.json"), '{"name":"fixture-peer","version":"1.0.0"}\n');
      await writeFile(join(candidateRoot, "package.json"), '{"name":"fixture-candidate","version":"1.0.0","type":"module"}\n');
      await writeFile(join(outside, "sentinel"), `${mode}-outside\n`);
      await writeFile(join(candidateRoot, "index.js"), [
        "import { existsSync, readdirSync, renameSync, symlinkSync } from 'node:fs';",
        "import { join } from 'node:path';",
        "import { tmpdir } from 'node:os';",
        "const root = process.env.FOUNDRY_HOSTILE_CONSUMER; const outside = process.env.FOUNDRY_HOSTILE_OUTSIDE;",
        "if (process.env.FOUNDRY_HOSTILE_MODE === 'consumer') { renameSync(root, join(outside, 'moved-consumer')); symlinkSync(outside, root, 'dir'); }",
        "if (process.env.FOUNDRY_HOSTILE_MODE === 'node-modules') { const nodeModules = join(root, 'node_modules'); renameSync(nodeModules, join(outside, 'moved-node-modules')); symlinkSync(outside, nodeModules, 'dir'); }",
        "if (process.env.FOUNDRY_HOSTILE_MODE === 'backup') { const backup = readdirSync(tmpdir()).find((name) => name.startsWith('foundry-aggregate-omitted-') && existsSync(join(tmpdir(), name, '0', 'package.json'))); if (!backup) throw new Error('backup not found'); const path = join(tmpdir(), backup); renameSync(path, join(outside, 'moved-backup')); symlinkSync(outside, path, 'dir'); }",
        "await import('fixture-peer').catch(() => {});",
      ].join("\n"));
      await assert.rejects(() => runAggregateOptionalPeerMatrix({
        consumer: temporary,
        matrix: [{ name: "@clossys/fixture", version: "1.0.0", peers: [{ peer: "fixture-peer", outcomes: { "fixture-candidate": "imports" } }] }],
        env: { PATH: process.env.PATH, TMPDIR: process.env.TMPDIR, FOUNDRY_HOSTILE_MODE: mode, FOUNDRY_HOSTILE_CONSUMER: temporary, FOUNDRY_HOSTILE_OUTSIDE: outside },
      }), (error) => {
        assert.match(error.message, /replaced|backup root/, `${mode}: ${error.message}`);
        return true;
      });
      assert.equal(await readFile(join(outside, "sentinel"), "utf8"), `${mode}-outside\n`);
    } finally {
      await rm(temporary, { recursive: true, force: true });
      await rm(join(outside, "moved-consumer"), { recursive: true, force: true });
      await rm(join(outside, "moved-node-modules"), { recursive: true, force: true });
      await rm(join(outside, "moved-backup"), { recursive: true, force: true });
      await rm(outside, { recursive: true, force: true });
    }
  }
});

test("optional-peer import and Next build reap normally exiting descendant groups", { skip: process.platform === "win32" }, async (t) => {
  const temporary = await mkdtemp(join(tmpdir(), "foundry-aggregate-optional-process-group-"));
  t.after(() => rm(temporary, { recursive: true, force: true }));
  const peerRoot = join(temporary, "node_modules", "fixture-peer");
  const importRoot = join(temporary, "node_modules", "fixture-import");
  const buildRoot = join(temporary, "node_modules", "fixture-build");
  const bin = join(temporary, "node_modules", ".bin", "next");
  const importMarker = join(temporary, "import-descendant-marker");
  const buildMarker = join(temporary, "build-descendant-marker");
  const delayedWriter = (marker) => `setTimeout(() => require('node:fs').writeFileSync(${JSON.stringify(marker)}, 'unexpected'), 350);`;
  await mkdir(peerRoot, { recursive: true });
  await mkdir(importRoot, { recursive: true });
  await mkdir(buildRoot, { recursive: true });
  await mkdir(join(temporary, "node_modules", ".bin"), { recursive: true });
  await writeFile(join(temporary, "package.json"), '{"private":true}\n');
  await writeFile(join(temporary, "package-lock.json"), '{"lockfileVersion":3}\n');
  await writeFile(join(peerRoot, "package.json"), '{"name":"fixture-peer","version":"1.0.0"}\n');
  await writeFile(join(importRoot, "package.json"), '{"name":"fixture-import","version":"1.0.0","type":"module"}\n');
  await writeFile(join(importRoot, "index.js"), `import { spawn } from 'node:child_process'; const child = spawn(process.execPath, ['--eval', ${JSON.stringify(delayedWriter(importMarker))}], { stdio: 'ignore' }); child.unref(); export {}\n`);
  await writeFile(join(buildRoot, "package.json"), '{"name":"fixture-build","version":"1.0.0","type":"module"}\n');
  await writeFile(join(buildRoot, "index.js"), "export {};\n");
  await writeFile(bin, `#!/usr/bin/env node\nconst child = require('node:child_process').spawn(process.execPath, ['--eval', ${JSON.stringify(delayedWriter(buildMarker))}], { stdio: 'ignore' }); child.unref(); process.exit(0);\n`);
  await chmod(bin, 0o755);

  const observations = await runAggregateOptionalPeerMatrix({
    consumer: temporary,
    matrix: [{ name: "@clossys/fixture", version: "1.0.0", peers: [{ peer: "fixture-peer", outcomes: { "fixture-import": "imports", "fixture-build": "imports" } }] }],
    frameworkByPackage: new Map([["@clossys/fixture@1.0.0", { client: [], server: [], proxy: [], all: ["fixture-build"] }]]),
    env: { PATH: process.env.PATH },
  });
  assert.equal(observations.length, 2);
  assert.equal(JSON.parse(await readFile(join(peerRoot, "package.json"), "utf8")).name, "fixture-peer");
  await new Promise((resolveResult) => setTimeout(resolveResult, 700));
  await assert.rejects(() => readFile(importMarker), { code: "ENOENT" });
...
test("first-party runtime joins resolve Publisher siblings to frozen top-level physical identities", async () => {
  const selected = record.sets[0].packages.filter((entry) => ["publisher", "controller", "writer", "designer"].includes(entry.packageKey));
  const writePackage = async (temporary, directoryName, manifestName, version, manifest = {}) => {
    const directory = join(temporary, "node_modules", ...directoryName.split("/"));
    await mkdir(directory, { recursive: true });
    await writeFile(join(directory, "package.json"), `${JSON.stringify({ name: manifestName, version, ...manifest })}\n`);
    return directory;
  };
  const clean = await mkdtemp(join(tmpdir(), "foundry-aggregate-runtime-joins-clean-"));
  const hostile = await mkdtemp(join(tmpdir(), "foundry-aggregate-runtime-joins-hostile-"));
  try {
    const publisherManifest = { dependencies: { "@clossys/controller": "~0.8.0", "@clossys/writer": "^0.3.0", "@clossys/designer": "^0.2.4" } };
    for (const entry of selected) await writePackage(clean, entry.name, entry.name, entry.version, entry.packageKey === "publisher" ? publisherManifest : {});
    const deduped = await validateFirstPartyRuntimeJoins(clean, selected);
    assert.deepEqual(deduped.findings, []);
    assert.deepEqual(deduped.resolutions.map((item) => item.physical), ["$CONSUMER/node_modules/@clossys/controller", "$CONSUMER/node_modules/@clossys/designer", "$CONSUMER/node_modules/@clossys/writer"]);

    for (const entry of selected) await writePackage(hostile, entry.name, entry.name, entry.version, entry.packageKey === "publisher" ? publisherManifest : {});
    await writePackage(hostile, "@clossys/publisher/node_modules/@clossys/writer", "@clossys/writer", selected.find((entry) => entry.packageKey === "writer").version);
    await writePackage(hostile, "@clossys/publisher/node_modules/@clossys/designer", "@clossys/designer", selected.find((entry) => entry.packageKey === "designer").version);
    const nested = await validateFirstPartyRuntimeJoins(hostile, selected);
    assert.deepEqual(nested.findings.map((item) => item.rule), ["runtime-resolution", "runtime-resolution"]);
    assert.equal(nested.resolutions.find((item) => item.dependency === "@clossys/writer").physical, "$CONSUMER/node_modules/@clossys/publisher/node_modules/@clossys/writer");
  } finally {
...
test("aggregate CLI treats a missing HEAD closure as unavailable but a present non-blob as violated", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "foundry-aggregate-cli-head-"));
  const script = join(root, "scripts", "run-public-npm-aggregate-canary.mjs");
  const closurePath = `governance/public-npm-aggregate-closures/baseline-${"a".repeat(64)}.json`;
  try {
    execFileSync("git", ["init", "-q"], { cwd: temporary });
    await mkdir(join(temporary, closurePath), { recursive: true });
    await writeFile(join(temporary, closurePath, ".gitkeep"), "directory, not a closure blob\n");
    execFileSync("git", ["add", "-A"], { cwd: temporary });
    execFileSync("git", ["-c", "user.name=Test", "-c", "user.email=test@example.invalid", "commit", "-qm", "directory closure"], { cwd: temporary });
    const nonBlob = spawnSync(process.execPath, [script, "--closure", closurePath], { cwd: temporary, encoding: "utf8" });
    assert.equal(nonBlob.status, 1); assert.match(nonBlob.stderr, /VIOLATED/);
    const missing = spawnSync(process.execPath, [script, "--closure", `governance/public-npm-aggregate-closures/baseline-${"b".repeat(64)}.json`], { cwd: temporary, encoding: "utf8" });
    assert.equal(missing.status, 2); assert.match(missing.stderr, /INDETERMINATE/);
  } finally { await rm(temporary, { recursive: true, force: true }); }
});
...
test("deterministic synthetic aggregate uses one local-tarball consumer for all nineteen child dispatches and real rollback", async () => {
  ...
  const plan = structuredClone(record); plan.peerResolution = { requested: {}, disposition: [] }; plan.optionalPeerMatrix = plan.optionalPeerMatrix.map((row) => ({ ...row, peers: [] }));
  const selected = plan.sets[0];
  ...
  try {
    ...
    for (const entry of selected.packages) {
      ...
      const manifest = { name: entry.name, version: entry.version, type: "module", ...(["publisher", "builder", "inspector"].includes(entry.packageKey) ? { dependencies: { "@clossys/controller": "*", ...(entry.packageKey === "publisher" ? { "@clossys/writer": "*", "@clossys/designer": "*" } : {}) } } : {}) };
...
test("satisfied records bind the closed plan, closure, canonical payload, and one immutable introduction", () => {
  ...
  const missingRuntimeEdge = structuredClone(transcript); missingRuntimeEdge.consumer.runtimeResolutions = missingRuntimeEdge.consumer.runtimeResolutions.filter((item) => item.from !== "@clossys/builder@0.7.3");
  assert.ok(validateSatisfiedAggregateTranscript(missingRuntimeEdge, { plan, closure }).some((item) => item.rule === "rollback"));
