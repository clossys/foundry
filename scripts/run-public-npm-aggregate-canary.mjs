...
try {
  ...
  if (closurePath) {
    ...
    let mode;
    try {
      mode = execFileSync("git", ["ls-tree", "HEAD", "--", closurePath], { cwd: process.cwd(), encoding: "utf8" });
    }
    ...
    if (mode.trim() === "") throw new AggregateCanaryCliError("immutable closure is unavailable: no such HEAD path", 2);
    if (!mode.startsWith("100644 ")) throw new Error("immutable closure is not a regular HEAD blob");
    bytes = execFileSync("git", ["show", `HEAD:${closurePath}`], { cwd: process.cwd(), encoding: "utf8" });
