...
// is judged too, as one extra result: exit 1 if it changed beyond those
// bumps' version and range edits, exit 2 if it changed on a positional
// (partial) run that cannot see every bump. See main().
//
...
function evaluatePackage(pkgDir, requestedBase) {
  ...
  // follow a consumed changeset -- the release-PR case, and the only case
  // whose lockfile main() judges.
...
function main() {
  ...
  // rewrites) of the bumps its consumed changesets justify, and anything
  // else means the lockfile was regenerated by the wrong npm. A direct,
  // changelog-justified bump is a different case -- it may legitimately add
  // or change a dependency in the same pull request -- so when no bump in
  // this diff consumed a changeset, no lockfile verdict is added and the
  // result is exactly what it was before this check existed. When one did,
  // only changeset-consumed bumps justify lockfile edits; a direct bump
  // mixed into a release commit does not. package-lock.json is one file
  // covering every workspace package, so a CHANGED lockfile is judged only
  // on a full discovery run: a positional package subset cannot see every
  // bump that might account for an edit, so it is refused (exit 2) rather
  // than judged against a partial set. See
  // scripts/lib/release-pr-lockfile-shape.mjs.
  const releaseBumps = [];
  for (let i = 0; i < targets.length; i += 1) {
    const r = results[i];
    if (r.versionChanged !== true || r.changesetConsumed !== true) continue;
    let manifest;
    try {
  ...
  if (releaseBumps.length > 0) {
    ...
    // of them will do.
    const { gitRoot, mergeBase } = results.find((r) => r.versionChanged === true && r.changesetConsumed === true);
