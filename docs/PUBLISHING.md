...
the owner-wide check remains mandatory: the failure mode is silent and hard to
undo cleanly.
...
with `GH_PACKAGES_TOKEN` set in the environment. The consuming plane owns that
credential reference and its local or CI injection. Foundry never stores the
consumer's value or installation manifest.
...
## 7. Canonical registry qualification
