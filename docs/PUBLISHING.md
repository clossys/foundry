...
The required `candidate qualification records` CI check re-derives every
retained record on each merge-group commit, so the publish workflow's `qualify`
job re-derives only the dispatched package's current-version record
