...
- **Status:** Accepted 2026-09-25. Documentation only: this file
...
- `derived: true` marks a file whose bytes are checked by `invariants`,
  ...
  rule checks `package-lock.json` or `pnpm-lock.yaml` without the set
  naming a package-manager-specific path inside `node_modules`. A Yarn
  repository is not covered by this invariant today: it is `indeterminate`
  until Starter supports Yarn and an integrity check for `yarn.lock`
  exists, and this RFC does not treat `yarn.lock` as equivalent to the
  other two lockfiles in the meantime. A lockfile regenerated by a
  different npm minor version would otherwise break idempotency for no
  safety gain. See D7.
...
## 13. Decisions from the owner (all recommended options accepted)
...
- **D1. Adopt the split in section 5.** Packages compute, materialize and
  ...
  pushes through `gh`). Decided: the split.
- **D2. What authorizes a staffing-only bundle** (brief, skills, layout, no
  ...
  digest-free `chosen: "approved"` for staffing-only sets. Decided: (a).
...
- **D3. Two pull requests on a repository's initial application** (setup,
  ...
  until a later pull request. Decided: two, and one per repository
...
- **D6. Superseded pull requests.** Open a new branch and pull request per
  ...
  branch. Neither force-pushes. Decided: a new branch per digest, which
...
- **D7. Lockfile policy.** Treat lockfiles as `derived` (semantic
  ...
  byte-identical lockfiles under a pinned npm version. Decided:
...
- **D8. Where the plan and brief contracts live.** Launcher currently has
  ...
  depends on `@clossys/advisor` and imports its validators. Decided:
...
- **D9. Where installed state lives** (12.2). Options: (a) a ledger
  ...
  recomputed from plan history and the tree. Decided: (a). It changes
...
- **D10. How ownership is recorded** (12.1). Options: (a) digests in the
  ...
  markers inside the files. Decided: (a). With (b), JSON files cannot
...
- **D11. Removal of a client-edited file** (12.5). Options: (a) refuse that
  ...
  (b) delete it anyway; (c) three-way merge. Decided: (a). With (b),
...
- **D12. Role records when a role is removed** (12.5). Options: (a) leave
  ...
  the rule says today. Decided: (a). With (b), a removal deletes
...
- **D13. What repair may be proposed without the client asking** (12.6).
  ...
  separate line item; (b) any drift on an owned path. Decided: (a).
...
- **D14. An update that overlaps an unmerged earlier pull request** (12.7).
  Options: supersede, stack, or refuse. Decided: supersede, and hold
...
- **D15. Batching and cadence** (12.7). Options: (a) one approval per
  ...
  update as it arrives. Decided: (a). With (b), a client with several
...
- **D16. Standing approvals for low-risk updates** (12.7). Options: (a)
  ...
  example "patch updates of installed packages". Decided: (a). With
...
- **D17. How a package change is proved by Starter** (12.3). Options: (a)
  ...
  work; (c) describe the apply pull request as Starter-proved. Decided:
...
- **D18. A repository removed from the inventory** (12.4). Options: (a)
  ...
  separate, explicit item; (b) treat it as an uninstall. Decided: (a).
...
- **D19. Repository identity** (12.4). Options: (a) record GitHub's
  ...
  only. Decided: (a). With (b), a rename followed by someone taking the
...
- **D20. Provenance for installed versions** (V9, 12.4). Integrator's
  ...
  Decided: (a). With (b), a brand-new package can never be installed.
...
D27 to D33 are new. Like D21 to D26, these arrived from the owner already
decided, not as open options.

- **D27. Root-entry declaration for a closed vocabulary.** When a target
  repository's Controller repository profile declares a closed
  `rootEntries` vocabulary, the setup set will gain a declare-root-entry
  act: it adds the `clossys` root entry, with the classification and
  disposition the profile's own rules assign it, through the same
  JSON-pointer key edit the plan's other keyed writes use (4.3: only that
  key changes), so the profile and the tree agree. A plan whose target
  profile cannot be located or parsed will be `indeterminate`.
- **D28. A symlinked skills root is refused.** If the skills directory, or
  any ancestor of a path a change set writes, is a symlink at the base
  commit, the set will be `indeterminate` rather than written through the
  link, because a write through a symlink can land outside the repository
  the plan approved.
- **D29. Untracked files count as a dirty tree.** The dirty-base check
  will treat untracked files as dirty, the same as modified tracked ones,
  and will name any files a recognized skill path pattern matches as
  leftovers specifically, so a client sees they were left by an earlier
  run rather than reading an unexplained diff. Nothing is deleted to clean
  them.
- **D30. A consumer skill-namespace carve-out, not a rename.** Foundry
  will publish the exact path patterns its own packages own, as a
  declared contract, and each consumer decides for itself, in its own
  skill policy, to carve out that namespace. Foundry adds no
  consumer-specific code for this; the carve-out lives entirely on the
  consumer side.
- **D31. A machine-opened pull request links a task-record issue.**
  Before a setup or apply pull request is opened in a target repository,
  the agent procedure will file one labelled issue in that same target
  repository recording the plan being installed, and will link the setup
  and apply pull requests to it from a `## Task record` section in each
  pull request body. No pull request is exempt from carrying this link.
  The label used comes from the target repository's own task-record
  configuration, read by the agent, not fixed by Foundry.
- **D32. Scope comes from staffing, not from inventory.** Every step of
  applying a plan — snapshot, planning, materialize, verify, the
  validation table, opening pull requests, and status — will operate only
  on the repositories a plan actually staffs. An inventoried but unstaffed
  repository will never be read for gating, never written, never cloned,
  and never degrades or blocks a run over its own state (a dirty tree, a
  stale pin, or anything else).
- **D33. A staffed repository gets the Advisor voice plus its staffed
  roles' voices.** A repository's setup set will compose the Advisor
  voice together with the voices of every role staffed in that
  repository; an unstaffed repository gets none of them. A voice's
  absence is a defect only in the hub, which is expected to carry every
  voice regardless of what any one repository staffs.
