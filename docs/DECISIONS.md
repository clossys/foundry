...
answer. The other half was never written down and is universal in the code
anyway — a role package ships a gate behind a `bin`, so a consumer's own CI can
fail on it. Fifteen of the nineteen packages in this tree do. `auth`, `comms`
...
cut; a third is named and unbuilt; a fourth is named so its absence is a
...
`packages/comms/src/dispatcher.ts:57` reads:
