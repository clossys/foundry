...
All notable changes to `@clossys/builder` are documented here.
...
- No packed content changed. This package's test suite changed as part of
  ...
  new version. Renumbered repeatedly as main moved ahead during this
  restack's disk-incident hold (0.9.1 -> 0.10.1): main independently
  shipped the web deployment surface (issue #1211) as 0.10.0, ahead of
  this test-only bump (version-collision rule, issue #1187).
...
- Patch version bump only, to obtain a fresh, never-before-used
  `governance/release-qualifications/` record path. The 0.7.7 qualification
  ...
  be introduced at the `0.7.7` path, so this package moves to `0.7.8`
...
- Hardened both Builder command-line Markdown table renderers so untrusted cell
...
- **Class 1 (package-owned, account-neutral conventions content) now
  ...
  `@clossys/controller/conventions` catalog id onto a
...
- **`aggregate-observations`, `check-observation-freshness`, and
  ...
  `@clossys/controller/gates`'s own `gateResultToExitCode`;
...
- **BREAKING: `createVercelInspector().inspect` and `createRenderInspector().inspect`
  ...
  `@clossys/integrator`'s `resolveReachability` already applies to
...
- **Machine composition (#393), a new `./machine` subpath.** Composes one
  ...
  `@clossys/controller/conventions`'s own `SkillScope` vocabulary.
...
- **Widened the declared `@clossys/controller` dependency range to
  `~0.8.0`** so it covers controller's issue #391 release (required-check
  recency grading). This package does not use `@clossys/controller/review`
...
- **Observation-bundle transport (#255, narrowed to the aggregation
  ...
  - `ObservationBundle` / `OBSERVATION_BUNDLE_SCHEMA_VERSION` -- one
    ...
    `@clossys/controller/gates`'s existing ternary rather than a
...
- The `liveStateSurface` finding-kind section now matches
  `@clossys/controller`'s corrected description (#313): the five
...
- **`live-state.ts` now re-exports `@clossys/controller/conventions`'s
...
- `packages/builder`, absorbing the previous scope's `provisioning` at its
  root entrypoint and that scope's `deployment` as the `./deployment`
...
- `liveStateSurface`: `LiveStateSurfaceDeclaration`,
  ...
  built on `@clossys/controller/gates`'s `GateResult` ternary.
...
- `./ci`: shared CI gate mechanics for #257 — `foldLiveStateReports`,
  ...
  mechanism the previous scope's `verify-standards` already ships), and
...
[0.1.0]: https://github.com/clossys/foundry/releases/tag/builder-v0.1.0
